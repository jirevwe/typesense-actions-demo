...
func (p *Person) toGenericMap(document *map[string]interface{}) error {
...
func Test_Search(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			pp, _, err := ts.Search(testCollection, &utils.SearchFilter{
				Query: tt.query,
				...
				Pageable: utils.Pageable{Page: 1, PerPage: 10, Sort: 1},
			})
			require.NoError(t, err)
